...
def aoc2018_04_1(filename):
    if __name__ != "__main__":
        print("\nAoC 2018, Day 04, Part 1\n~~ running as a test ~~")
    ...
    max_minutes_for_all_guards = 0
    most_sleepy_guard = 0
    for guard in list_of_guards:
        sleep_time = list_of_guards[guard].countAllSleeps()
        if sleep_time > max_minutes_for_all_guards:
            #print(guard)
            most_sleepy_guard = guard
            max_minutes_for_all_guards = sleep_time
...
class GuardSchedule:
    ...
    def minuteMostAlseep(self):
        ...
        return max(self.hourDict, key=self.hourDict.get)
