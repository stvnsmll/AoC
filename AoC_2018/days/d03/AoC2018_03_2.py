...
def aoc2018_03_2(filename):
    ...
    #check every cell of each fabric map to see if an overlap was every placed on it
    for fabricmap in all_fabrics:
        x_offset = fabricmap.fromLeft
        y_offset = fabricmap.fromTop
        #first go by rows
        for x_pos in range(fabricmap.width):
            for y_pos in range(fabricmap.height):
                global_x_pos = x_offset + x_pos
                global_y_pos = y_offset + y_pos
                if full_fabric_map[global_y_pos, global_x_pos] == "O":
                    fabricmap.noOverlap = False
                    break

    for fabricmap in all_fabrics:
        #print(f"{fabricmap.name} no overlap: {fabricmap.noOverlap}")
        if fabricmap.noOverlap == True:
            print(f"Runtime Duration: {(datetime.now() - startTime)}\n")
            print(f"Elf with no overlap was found: {fabricmap.name}")
            return int(fabricmap.name)
