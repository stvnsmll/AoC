# -*- coding: utf-8 -*-
"""
Created on Tue Dec  3 14:36:25 2019
...
def plot_coords(wire_instructions):
    coord_count = len(wire_instructions)
    full_path = [[0,0]]
    i = 0
    for coord in wire_instructions:
        #i is the coordinate position in wire_instructions
        #Direction
        direction = coord[:1]
        magnitude = coord[1:]
        
        #pathstartx = full_path[i][i]
        startx = full_path[i][0]
        starty = full_path[i][1]
        
        #print("start at x = " + str(startx) + " and y = " + str(starty) + ".")
        
        
        #print("Go " + direction + " " + magnitude + ". (pos " + str(i) + ")")
        #print(i)
        #print(magnitude)
        
        for j in range(int(magnitude)):
            #print(j+1)
            if direction == "L":
                startx -= 1
            if direction == "R":
            ...
            if direction == "D":
                starty -=1
            #print("->end at x = " + str(startx) + " and y = " + str(starty) + ".")
            full_path.append([startx,starty])
            #print(full_path)
        i = (len(full_path)-1) #to get it to start the next direction using the last saved coordinate
    return full_path

wire1_fullpath = plot_coords(wire1_inst)
wire2_fullpath = plot_coords(wire2_inst)

print()
print()
print("Outputs: ")
#print(wire1_fullpath)
#print(wire2_fullpath)

print("Wire 1 Steps: " + str(len(wire1_fullpath)))
print("Wire 2 Steps: " + str(len(wire2_fullpath)))

alldistances = []
allsteps = []
i = 1
for position1 in wire1_fullpath:
    #print(str(i) + ": " + str(position1))
    j = 1
    for position2 in wire2_fullpath:
        if position1 == position2:
            distance = abs(position1[0]) + abs(position1[1])
            stepcount = (i + j)-2
            alldistances.append(distance)
            allsteps.append(stepcount)
            print("Found match: " + str(i) + str(position1) + str(j) + str(position2) + "  >>Distance = " + str(distance) + "  >>Steps = " + str(stepcount))
        j+=1
    i+=1
...
print(alldistances)
print(allsteps)
mindistance = alldistances[1]
minsteps = allsteps[1]

print("The minimum distance to an intersection is: " + str(mindistance))
print("The minimum steps to an intersection is: " + str(minsteps))